...
export class CacheService implements OnDestroy{
    ...
    public setSideBar(package_id:any,user_id:any,data:any):void{
    	let key = 'menu_'+package_id+'_'+user_id;
    	localStorage.setItem(key,JSON.stringify(data));
    }

    public getSideBar(package_id:any,user_id:any){
    	let key = 'menu_'+package_id+'_'+user_id;
    	// localStorage.getItem(key,data);
    	// localStorage.getItem(key);
        let retStr:string = localStorage.getItem(key);
    	return retStr;
        // if(retStr.length>0){
